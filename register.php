...
if (isset($_POST['submit'])) {
    ...
    $chk = "SELECT * FROM users_details WHERE email = '$email'";
    $result = $mysqli->query($chk);
    while ($row = $result->fetch_assoc()) {
        $user_username = $row['username'];
        $user_email = $row['email'];
    }
    if ($email === $user_email || $username === $user_username) {
        $postUpdate =
            '<div class="alert alert-danger" role="alert">Account already exist, use a different email/username or reset password</div>';
    } else {
        $sqli = "INSERT INTO users_details (username, email, user_password, referral_code) VALUES ('$username', '$email', '$password', '$referral_code')";
        $sql = "INSERT INTO users (email, user_password) VALUES ('$email', '$password')";
        if ($mysqli->query($sqli) === true && $mysqli->query($sql) === true) {
            $postUpdate =
                '<div class="alert alert-dark" role="alert">Account created successfully</div>';
        } else {
...
<body style="background: #f3f3f3;">
    <div class="m-auto login-container container">
        ...
        <form action="<?php echo htmlspecialchars(
            ...
            <input type="text" name="username" id="" class="form-control mb-3" required placeholder="create username">
            ...
            <input type="hidden" name="referer_code" id="" class="form-control mb-3">
